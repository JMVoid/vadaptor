...
func (s *DbClient) PullUser(nodeId uint32) (userRepo *pb.UserRepo, err error) {
	...
	for rows.Next() {
		...
		log.Debugf("%s-%s-%d-%d-%d-%d-%d-%d", user.Username, user.Uuid, user.AlterId, user.Level, user.Enable, user.TransferEnable, user.Uplink, user.Downlink)
...
func (s *DbClient) PushUserTransfer(userList []pb.User) error {
	var queryWhend string
	var queryWhenu string
	var sqlWhen string
	var sqlWhere string
	...
	db, err := sql.Open("mysql", s.SrvCfg)
	if err != nil {
		log.Errorf("fail to open mysql server with : %s", err.Error())
		return err
	}
	...
	stmt, err := db.Prepare(sqlRun)
	if err != nil {
		log.Errorf("Push user prepare error:%s", err.Error())
		return err
	}
